...
- Continue the reliable single-page render harness over the remaining `345` pages, or split by offset into resumable batches that launch one page per xcodebuild invocation.
...
- Combined latest-current proof across maintained result files is now `175` unique pages, `525` screenshots, and `0` current failures.

Remaining honest risks:

- The global V2.2 rendered proof gate is still open: `345` pages still need current screenshots unless the gate is explicitly amended.
