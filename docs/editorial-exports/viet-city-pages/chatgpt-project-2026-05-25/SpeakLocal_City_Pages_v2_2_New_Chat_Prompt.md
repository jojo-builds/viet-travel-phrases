...
If Codex provides a `humanizer-gate-500` JSON chunk prompt, follow that prompt instead of this normal readable-review flow. For that lane, repair exactly the supplied 25-entry chunk, preserve the exact JSON shape and existing phrase IDs, output one complete fenced JSON chunk, then add the writer self-gate. Do not pick replacement listings, change page IDs, add one-off phrase cards, or convert the chunk into a readable review memo.
...
For each listing, choose one owned traveler moment and one truthful story spine before writing. The story spine is the small local, cultural, city, route, food, neighborhood, or place-memory reason this page belongs here. Weave it into the intro or one early section; do not add a literal `Story` section unless the page naturally needs it.
...
- Do not wrap phrase-card Vietnamese in quotation marks unless the phrase truly contains a quote.
- Keep `check_catalog`, `not_run`, scores, source notes, freshness risks, QA notes, and schema/process terms out of Reader View.
- Do not put generic export labels like `Sections` or bare `Phrase cards` into app-visible fields. If you need a review label, keep it outside Reader View; if the app needs a phrase-card label, use `Useful phrase cards`.
...
- If a page reads as clean utility but carries no story, revise the first screen before final output.
- If the same three phrase cards repeat from another page in this batch, swap one card for a better reusable ready-audio fit, justify the repeat, or mark the page as needing revision.
- Run a cadence pass before final output. If headings cluster around the same command verbs (`Keep`, `Choose`, `Pick`, `Pair`, `Leave`, `Read`, `Watch`, `Ask`, `Confirm`), revise the cluster into more observed, page-specific headings.
- If the row is thin-source, shrink the claim or mark it as blocked. Do not fill gaps with generic props like signs, benches, bags, awnings, shopfronts, or "confirm the name" unless the evidence supports that actual page.
