...
- `node native-ios/scripts/audit-visible-product-language.js` blocks retired visible product labels such as old Messages/conversation wording on active Practice-era surfaces, including direct SwiftUI labels, selected model/source-backed strings, case variants such as `MESSAGES`, nested completion labels such as `Conversation complete`, and one-line accessibility fallbacks such as `Conversation break`.
