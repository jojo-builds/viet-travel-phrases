...
- current app-code payload `521cdf883` built and installed successfully on Jojo's active physical iPhone after the late eight-hour follow-up. iOS refused launch because the phone was locked; repo signing files stayed clean and no project signing settings were changed.
...
- second product-language smell pass removed non-text Messages-era cues from Practice: `checkmark.message.fill` -> `checkmark.seal.fill` on the Practice completion surface, and `without letting the thread drift` -> `without losing the plan` in the driver-help scenario setup. The visible-language audit now blocks the exact retired visual symbol `checkmark.message.fill` and exact stale phrase `thread drift`; fresh validation passed with visible-language audit, audit unit test, Swift parse of the changed Swift files, JS syntax checks for the audit script and test, exact `rg` scan showing only guardrail/test references, and `git diff --check`.
- after the Practice product-language guidance update, the latest `main` reran `ProductLanguageUITests/testRepresentativeRoutesDoNotExposeRetiredPracticeVocabularyWhileScrolling` successfully: `1` test, `0` failures, `280.515` seconds. This rerun covered Home, Browse, Local Greetings, Eating Out, Airport, Hotel, Hoi An, Da Nang, Search, Saved, and Practice after the hidden fallback, visual-symbol, and source-of-truth guidance cleanups.
- exact-current phone install proof was refreshed after that route-scrolling proof: app-code payload `521cdf883` built and installed on Jojo's active physical iPhone, launch was blocked only because the phone was locked, and tracked signing files stayed clean.
- final eight-hour closeout reran broad current-main shell `xcodebuild` front-end slices after the XcodeBuildMCP test transport closed: `BrowseSearchUITests` passed `68` tests with `0` failures (`/tmp/speaklocal-eight-hour-browse-search.xcresult`), `AdminChromeUITests` passed `30` tests with `1` intentional skip and `0` failures (`/tmp/speaklocal-eight-hour-admin-chrome.xcresult`), `PracticeUITests` passed `13` tests with `0` failures (`/tmp/speaklocal-eight-hour-practice.xcresult`), and the regression cluster passed `18` tests with `0` failures across audio tap reliability, back/forward/swipe navigation, bottom-inset geometry, and product-language route scrolling (`/tmp/speaklocal-eight-hour-regression-cluster.xcresult`). The final route-scrolling guard again covered Home, Browse, Local Greetings, Eating Out, Airport, Hotel, Hoi An, Da Nang, Search, Saved, and Practice.
- after the post-closeout static sweep, the product-language route scroller was rerun once more from current `main` and passed: `ProductLanguageUITests/testRepresentativeRoutesDoNotExposeRetiredPracticeVocabularyWhileScrolling`, `1` test, `0` failures, `279.335` seconds, result bundle `/tmp/speaklocal-final-product-language.xcresult`.
- post-closeout static gate sweep passed after the final UI proof: native-only guard, native chrome guard, visible product-language audit, visible audit unit test, SQLite fixture validation, audio manifest sync (`5353` entries), Tier 1 listing validation (`150` strong), Vietnamese menu copy/images (`355` pages and `355` image pairs), search-only surfacing (`315` rows), production listing QA (`1793` pages, `0` blockers, `0` majors), V2.2 city strict validation (`520` pass), normal hero image validation (`524` active premium hero assets), phrase backdrops (`952` placements), catalog-promoted authoring (`770` authored pages), breakdown audit (`PASS`), city library (`826` pages), city copy (`5` hubs, `520` city noun pages), listing intent routing, and `git diff --check`. A stale breakdown-audit test expectation was corrected to match the app-facing `tôi = I / me` and retained `keepTogetherReason` contract; the repaired script test group then passed `25` tests with `0` failures.
...
- Shell `xcodebuild` simulator UI test, `ProductLanguageUITests/testRepresentativeRoutesDoNotExposeRetiredPracticeVocabularyWhileScrolling`
  ...
  - passed again after the later visual-symbol and guidance cleanup: `1` test, `0` failures, `280.515` seconds
