...
Scope: docs-only homepage visual refresh packet.
...
| Native iOS / Liquid Glass fit | Pass with note. The comps use the current masthead, white glass cards, stable bottom dock/search island, and restrained red/green accents. Native implementation should use real SwiftUI materials rather than copying the raster effects exactly. |
| Reduced visual clutter | Pass. The core rule removes the competing decorative icon/audio/chevron stack from small phrase cards. |
...
Proceed with a native implementation task for the Hybrid Utility Home. The implementation should start by refactoring phrase-card affordances, then add a small image-led Browse collection layer, and only then add returning-user module refinements.
