...
export default function AppPreviewWireframesIndex() {
  return (
      ...
      <ScrollView className="flex-1" contentContainerClassName="gap-4 px-5 pb-12 pt-8">
        <View className="rounded-[32px] border border-border bg-surface p-5 shadow-sm">
          <ThemedText variant="label" className="text-primary">
            Native-first wireframe deck
          </ThemedText>
          <ThemedText variant="title" className="mt-2">
            App Preview concepts for {previewDeckContent.appName}
          </ThemedText>
          <ThemedText className="mt-3">
            Open a slide below to review the full-screen wireframe on device. These are route-level concepts for native capture,
            not final App Store screenshots yet.
          </ThemedText>
        </View>
        ...
        {appPreviewSlides.map((slide) => (
          <Pressable
            ...
            onPress={() => router.push(`/app-preview-wireframes/${slide.id}`)}
