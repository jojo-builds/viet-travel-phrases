  ...
  return (
    <SafeAreaView className="flex-1 bg-background px-5 pt-6" edges={['top', 'bottom']}>
      <View className="space-y-6">
        ...
        {(['About', 'Legal', 'Feedback'] as const).map((section) => <View key={section} className="space-y-3"><ThemedText variant="subtitle">{section}</ThemedText><View className="rounded-2xl border border-border bg-surface shadow-sm">{rows.filter((row) => row.section === section).map((row, index, list) => <Pressable key={row.label} accessibilityLabel={row.label} onPress={row.action} style={({ pressed }) => [{ opacity: pressed ? 0.7 : 1 }]} className={`flex-row items-center justify-between px-4 py-4 ${index < list.length - 1 ? 'border-b border-border' : ''}`}><ThemedText>{row.label}</ThemedText><ThemedText className="text-caption">›</ThemedText></Pressable>)}</View></View>)}
        <ThemedText variant="caption" className="pt-4 text-center">Version 1.0.0 • Made with ❤️ in Vietnam</ThemedText>
      </View>
    </SafeAreaView>
  );
}
