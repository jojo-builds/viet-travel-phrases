...
const canonicalPagesRoot = path.join(repoRoot, "content-draft", "viet", "canonical-pages");
const catalogPromotedRoot = path.join(canonicalPagesRoot, "catalog-promoted");
const indexPath = path.join(catalogPromotedRoot, "_catalog-promoted-index.json");
const rationalePath = path.join(catalogPromotedRoot, "_ai-authoring-rationale.jsonl");
const audioManifestPath = path.join(repoRoot, "native-ios", "Resources", "viet-audio-manifest.json");
// Historical task ID retained because source rows still use it for provenance.
...
function main() {
  assert(fs.existsSync(catalogPromotedRoot), `Missing catalog-promoted canonical authoring root: ${relative(catalogPromotedRoot)}`);
  assert(fs.existsSync(indexPath), `Missing catalog-promoted canonical index: ${relative(indexPath)}`);
  ...
  const pages = walkJSONFiles(catalogPromotedRoot).map((filePath) => ({
    filePath,
  ...
  for (const row of taskPhraseRows) {
    assert(pagesByPhraseID.has(row.phrase_id), `${row.phrase_id} is a task phrase without a catalog-promoted authored page`);
  }
  ...
  assert(index.taskID === taskID, "Catalog-promoted index taskID mismatch");
  assert(index.authoredPageCount === pages.length, "Catalog-promoted index authoredPageCount mismatch");
