...
## Title
Burn-in proof, heartbeat and lease observability audit

## Objective
Inspect whether the current heartbeat and lease fields give operators enough visibility to distinguish healthy long runs from abandoned ones.
...
- Task-local artifacts under `.agent/tasks/T-046/**` are the required output truth for this task.

## Required checks
- Use only read-only helper commands or file inspection beyond your own claimed task lifecycle.
- Record whether heartbeat timestamps, phases, and lease expiry tell a clear operator story.
...
## Definition of done
- Write a concise evidence log at `.agent/tasks/T-046/logs/heartbeat-lease-audit.md`.
