...
## Title
Burn-in proof, reclaim-resume breadcrumb completeness audit

## Objective
Judge whether reclaimed tasks now leave enough breadcrumbs for the next worker to resume decisively instead of rediscovering context.
...
- Task-local artifacts under `.agent/tasks/T-045/**` are the required output truth for this task.

## Required checks
- Compare the current recovery notes against the actual task states and files touched in round 1.
- Identify the minimum breadcrumb fields that should always exist after reclaimable tasks are resumed.
...
## Definition of done
- Write a concise evidence log at `.agent/tasks/T-045/logs/reclaim-breadcrumb-audit.md`.
