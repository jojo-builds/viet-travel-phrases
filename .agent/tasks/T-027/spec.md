...
## Title
Desktop Codex automation pilot, Japan second first-wave translation pass and unresolved-row reduction

## Objective
Build on the completed Japan first-wave translation pass by clearing the remaining unresolved high-value first-wave rows, then continuing directly into the next highest-value traveler rows so the lane leaves behind a materially larger chunk of prep-ready Japanese authoring truth instead of another tiny follow-up pass.

This is still prep-only work. Do not wire japanese into the runtime app.
...
- This task is successful only if the Japan lane leaves with materially stronger first-wave coverage than after T-011.
...
- exactly 4 review artifacts under .agent/tasks/T-027/reviews/
...
- then continue into the next highest-value traveler rows from `content-draft/japanese/phrase-source.csv` until at least `24` additional rows total were translated, materially upgraded, or consciously deferred with explicit reasons
...
- `phrase-source.csv` has materially stronger `target_text`, `pronunciation`, `notes`, and `status` coverage than it had after `T-011`
...
- which unresolved Japanese first-wave rows can now be fully cleared in this pass
...
- what the immediate next Japanese pass should do after this larger translation wave

## Mandatory 3-review-gate workflow
Use exactly 4 Codex subagent review roles in every gate:
1. 01-traveler-utility-review.md
2. 02-japanese-language-risk-review.md
...
- verify `content-draft/japanese/phrase-source.csv` still parses as CSV after edits
- verify `content-draft/japanese/first-wave-priority.csv` still exists and remains non-empty
- verify review evidence exists for Gate 1, Gate 2, and Gate 3 under `.agent/tasks/T-027/reviews/`
...
## Definition of done
- the japanese lane has materially stronger first-wave and immediate next-wave translation coverage than before this task
...
- `.agent/tasks/T-027/result.md` states what changed, what was verified, what remains open, and whether the objective was fully met

## Required result contract
Before stopping, write .agent/tasks/T-027/result.md with:
