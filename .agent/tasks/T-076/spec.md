...
## Title
Desktop Codex automation pilot, South Korea first-wave translation pack and prep-ready expansion pass

## Objective
Turn the South Korea lane from a newly authoring-ready scaffold into a real first-wave translation pack with materially stronger row coverage.
This task should clear the highest-value Korean rows first, then continue through the next coherent bounded batch so the lane leaves behind usable translation truth instead of another shortlist-only step.

This is still prep-only work. Do not wire Korean into the runtime app.
...
- `research/language-pipeline/korean/**`
...
- `T-063` already produced the South Korea lane scaffold, the ranked first-wave shortlist, and the rewrite notes needed to start real translation work.
...
- Keep politeness, script, romanization, and confidence posture honest where Korean traveler fit or sensitive wording still needs expert review.
...
- exactly 4 review artifacts under `.agent/tasks/T-076/reviews/` for each required gate
...
- clear the top-ranked unresolved Korean first-wave rows before expanding into lower-priority work
- then continue into the next highest-value traveler rows from `content-draft/korean/phrase-source.csv` until at least `24` row outcomes total were translated, materially improved, or consciously deferred with explicit reasons
...
- `phrase-source.csv` has materially stronger `target_text`, `pronunciation`, `notes`, and `status` coverage than it had after `T-063`
- `first-wave-priority.csv` reflects which high-value rows were cleared, which remain blocked, and what the next Korean pass should target

## What the task must decide
- which top-ranked Korean rows are strong enough to clear now
...
- what the immediate next Korean pass should do after this larger batch
...
- verify `content-draft/korean/phrase-source.csv` still parses as CSV after edits
- verify `content-draft/korean/first-wave-priority.csv` still exists and remains non-empty
- verify review evidence exists for Gate 1, Gate 2, and Gate 3 under `.agent/tasks/T-076/reviews/`
...
- verify the South Korea lane is more implementation-ready than before and no longer stops at scaffold-only truth

## Definition of done
- the South Korea lane has materially stronger first-wave translation coverage than before
- unresolved high-value Korean rows were cleared or consciously reclassified with explicit reasons
- the lane advanced by at least `24` row outcomes or proved that the bounded high-value candidate set was fully exhausted
- the next South Korea pass can start from a meaningfully smaller unresolved set without another broad orientation loop
- all 3 mandatory review gates passed with unanimous 4-subagent approval
- prep-only boundary is preserved and runtime wiring is untouched
- `.agent/tasks/T-076/result.md` states what changed, what was verified, what remains open, and whether the objective was fully met

## Required result contract
Before stopping, write `.agent/tasks/T-076/result.md` with:
